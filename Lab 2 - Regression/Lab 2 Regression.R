...
whitewineRFE = rfe(x = datawhite_norm[,1:11], y = datawhite_norm[,12], sizes = c(1:11), metrix = "RMSE", rfeControl = control)
print(whitewineRFE)
importance <- varImp(whitewineRFE, scale=FALSE)
print(importance)

###create new dataframe using top 5 predictors identified by random forest###
datawhite_predict= data.frame(datawhite_norm$trans.alcohol, datawhite_norm$trans.residual.sugar, datawhite_norm$trans.density,datawhite_norm$trans.volatile.acidity,datawhite_norm$trans.free.sulfur.dioxide,datawhite_norm$trans.quality)

#***********PLEASE TRY RUNNING MODEL ON 'datawhite_predict' dataset.********************
